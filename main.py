...
if not mydb:
    print("Failed to connect to database. Please ensure MySQL is running and database exists.")
    exit(1)
...
def view_user():
    ...
    if request.method=='POST':
        ...
        if act=="yes":
            cursor1 = mydb.cursor()
            cursor1.execute('SELECT * FROM sc_user WHERE id = %s AND password = %s', (id1,))
            ...
            cursor = mydb.cursor()
            cursor.execute('update sc_user set status=1 WHERE id = %s AND pass = %s', (id1,))
            mydb.commit()
        else:
            cursor = mydb.cursor()
            cursor.execute('update sc_user set status=0 WHERE id = %s AND pass = %s', (id1,))
            mydb.commit()
